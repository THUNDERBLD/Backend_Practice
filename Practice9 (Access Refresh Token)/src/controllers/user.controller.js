...
const updateUserAvatar = asyncHandler( async (req, res) => {
    // getting the avatar from the local storage which is present in the public/temp folder
    const avatarLocalPath = req.files?.avatar[0]?.path;
    if(!avatarLocalPath){
        throw new ApiError(400, "Invalid provided path of the avatar")
    }

    // uploading the new avatar of the user on cloudinary
    const avatar = await UploadOnCloudinary(avatarLocalPath);
    if (!avatar.url) {
    ...
    const publicIdOfCloudinaryImage = getCurrentUser.avatar.split("/")[avatarArrLastIndex].split(".")[0]          // getting the publicId of the old avatar
    ...
    // setting(updating) new avatar of the user
    const user = await User.findByIdAndUpdate(
        req.user?._id,
        {
            $set:{
